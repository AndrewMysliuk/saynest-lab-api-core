...
export const convertMessageToString = (
  message: string | ArrayBuffer | Buffer | Buffer[],
): string => {
...
const countTokens = (history: IConversationHistory[]): number => {
  ...
  return history.reduce(
    (acc, message) => acc + encoding.encode(message.content).length,
    0,
  );
};
...
export const trimConversationHistory = (
  ...
  while (
    countTokens([systemPrompt, ...trimmedHistory]) > max_tokens &&
    ...
    const lastUserIndex = trimmedHistory
      ...
      .find(
        ({ message }) =>
          message.role === "user" && message.pairId !== currentPairId,
      );
