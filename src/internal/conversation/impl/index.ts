...
export class ConversationService implements IConversationService {
  ...
  async processConversation(
    ...
    try {
      ...
      const historyArray = Array.isArray(initialHistory)
        ? initialHistory
        : [initialHistory];
      const conversationHistory = [...historyArray];

      const { transcription, user_audio_path } =
        await this.speachToTextService.whisperSpeechToText(
          whisper.audioFile,
          whisper?.prompt,
          sessionDir,
        );
      ...
      onData(
        "user",
        transcription,
        `/user_sessions/${activeSessionId}/${path.basename(user_audio_path)}`,
      );
      ...
      const trimmedHistory = await trimConversationHistory(
        conversationHistory,
      ...
      const toolCalls =
        gptResponse.choices[0]?.message?.tool_calls?.[0]?.function?.arguments;
      if (!toolCalls) {
      ...
      logger.error(
        `conversationService | error in processConversation: ${error}`,
      );
      throw error;
    }
  }
    ...
    if (session_id) {
      ...
      if (fs.existsSync(sessionDir)) {
        const conversationHistory =
          await this.historyRepo.getHistoryBySession(session_id);
