...
import {
  RealtimeAPIClientEventsEnum,
  RealtimeAPIServerEventsEnum,
} from "../../../types";
  ...
  openaiWS.on("open", () => {
    ...
    const initMessage = JSON.stringify({
      ...
      session: {
        ...
        instructions:
          "You are an English language assistant who provides answers in both text and audio...",
        temperature: 0.6,
        voice: "alloy",
      },
    });
  ...
  openaiWS.on("message", (messageFromOpenAI) => {
    ...
    switch (parsedJson.type) {
      case RealtimeAPIServerEventsEnum.CONVERSATION_ITEM_CREATED:
        openaiWS.send(
          JSON.stringify({ type: RealtimeAPIClientEventsEnum.RESPONSE_CREATE }),
        );
